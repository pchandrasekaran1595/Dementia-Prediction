...
class Model(nn.Module):
    def __init__(self, IL=None, HL=None, DP=None):
        ...
        if len(HL) == 1:
            ...
            self.model.add_module("FC2", WN(nn.Linear(in_features=HL[0], out_features=3)))
            ...
            self.model.add_module("FC3", WN(nn.Linear(in_features=HL[1], out_features=3)))
            ...
            self.model.add_module("FC4", nn.Linear(in_features=HL[2], out_features=3))
...
def dl_analysis(features=None, targets=None):
    ...
    if train_mode:
        tr_feats, va_feats, tr_trgts, va_trgts = train_test_split(features, targets, test_size=0.25,
                                                                random_state=u.SEED, shuffle=True, stratify=targets)
        ...
        precision, recall, f_score, _ = precision_recall_fscore_support(y_pred, va_trgts, labels=np.unique(y_pred))
